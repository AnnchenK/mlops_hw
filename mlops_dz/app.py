...
async def add_model(
    ...
    DVC(_logger).data_commit(
        body.dataset.X, body.dataset.y, path="./data", filename="train_data.json"
    )
...
async def my_list_models() -> Annotated[
    List[str], Field(description="the names of the models", examples=[model_names])
]:
    """
...
async def list_models() -> Annotated[
    Dict[str, str],
    Field(
        description="all the added models",
        examples=[{"68fb4ce6-24a8-4615-8830-61ccada86eba": "neural_network"}],
    ),
]:
    """
...
async def predict(
    id: Id = Query(
        description="id of the model", example="68fb4ce6-24a8-4615-8830-61ccada86eba"
    ),
...
async def alive() -> Annotated[
    Literal["yes"], Field(description="whether the service is alive")
]:
    """
